...
readonly DEFAULT_REPO="${MANAGED_AGENT_REPO_URL:-${RELEASEPANEL_RUNNER_REPO:-${RELEASEPANEL_RUNNER_REPO_HTTPS:-git@github.com:EdwardSoaresJr/releasepanel-runner.git}}}"
...
Env: MANAGED_AGENT_PANEL_URL (required), MANAGED_AGENT_RUNNER_KEY (optional), MANAGED_AGENT_ACCOUNT_KEY or RELEASEPANEL_AGENT_ACCOUNT_KEY (optional SaaS), legacy MANAGED_AGENT_PANEL_INSTALL_KEY / RELEASEPANEL_INSTALL_KEY, MANAGED_AGENT_INSTALL_ROOT, MANAGED_AGENT_REPO_URL (or RELEASEPANEL_RUNNER_REPO / legacy RELEASEPANEL_RUNNER_REPO_HTTPS for git clone URL — default is git@github.com:EdwardSoaresJr/releasepanel-runner.git)
EOF
}
