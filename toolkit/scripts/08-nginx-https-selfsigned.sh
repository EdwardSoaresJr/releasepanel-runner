...
require_root
site_env_arg="${1:-}"
if ! parse_deploy_env_as_first_arg "${site_env_arg}"; then
...
bash "${SCRIPT_DIR}/07-selfsigned-ssl.sh" "${site_env_arg}"
...
log "Writing HTTPS nginx config (self-signed) for ${RELEASEPANEL_SERVER_NAME}."
sed \
    ...
    "${RELEASEPANEL_TOOLKIT_DIR}/nginx/releasepanel-https-selfsigned.conf.example" > "${target}"
...
log "HTTPS nginx config (self-signed) installed."
